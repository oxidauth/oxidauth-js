...
export class OxidAuthClient {
  ...
  async exchangeToken(oldRefreshToken = null) {
    ...
    if (oldRefreshToken == null) {
      oldRefreshToken = await this.getRefreshToken();
    }

    if (!oldRefreshToken) {
      throw new OxidAuthError(
        "NO_REFRESH_TOKEN_FOR_EXCHANGE",
  ...
  async verifyToken(key) {
    ...
    if (token) {
      ...
      throw new OxidAuthError(
        "NO_TOKEN_FOUND_ERR",
        "no token was found, you may need to authenticate first",
      );
    }
  }
  ...
  async getPublicKeys() {
    ...
    if (this?._public_keys_exp_at < new Date() || !publicKeys) {
      ...
      this._public_keys_exp_at =
        Date.now() +
        (this._opts?.public_keys_ttl_secs ||
          DEFAULT_OPTS.public_keys_ttl_secs) *
          60 *
          1000;
    }
